...
Core test: is the assistant talking to ITSELF/the system (internal -> filter) or TO the user (user-facing -> keep)? A direct reply to something the user asked is user-facing EVEN IF it is about reminders/scheduling/how the assistant works.
...
Keep if: the text speaks TO the user (second person / imperative / warm display tone, often with emoji) - a greeting, meal reminder, encouragement, logged-meal card, progress bar, or dietary advice phrased AS advice ("go fat-free tonight", "try adding a veggie"). Meal/calorie NUMBERS alone do NOT make it internal: a user-facing card like "Breakfast logged! 111 kcal | Protein 5.8g" or "You've got ~1305 kcal for lunch" is KEEP. Also KEEP a direct conversational reply to something the user asked - INCLUDING explaining how reminders/scheduling/the assistant work when the user asked about it (e.g. user asks “你刚发的提醒原文是什么”, assistant replies “我给它的指令是…，你那边收到的是怎么写的?”). Such a reply is user-facing even though it mentions reminders/instructions/generation. The test is voice/addressee (who is spoken to), not the topic and not the presence of numbers. A casual spoken explanation given TO the user about why a reply was slow, or what the assistant found / could not find when the user asked (e.g. “刚才在查你问的提醒，发现那条发完就自动删了，所以慢了几秒” / “我翻了一圈，那条提醒已经清除了，找不到原文了”) is user-facing -> keep: spoken to the user in a conversational tone, not a private step log. Contrast a private step log addressed to no one (“Let me check the cron logs.” / “Searching sessions for the reminder…” / “hint_count >= 2, now compose”) which is internal -> filter.
