...
describe("handleMessageEnd", () => {
  ...
  it("keeps final-tag enforcement in message_end fallback", () => {
    ...
    // The gate ate the whole (untagged) reply: the turn ends as intentional
    // silence, never as empty text that the incomplete-turn guard would turn
    // into a user-visible harness error.
    expect(ctx.finalizeAssistantTexts).toHaveBeenCalledWith(
